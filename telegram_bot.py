...
FIELDS: list[tuple[str, str, str]] = [
    ...
    ("project_location", "Kawasan / lokasi projek", "Contoh: Petaling Jaya, Selangor"),
]
...
BOT_COMMANDS = [
    ...
    {"command": "drafts", "description": "Senarai draf belum siap"},
    {"command": "edit", "description": "Buka draf ikut ID"},
    {"command": "done", "description": "Selesai untuk langkah semasa"},
    {"command": "cancel", "description": "Batal draf semasa"},
...
def main() -> None:
    ...
    _run_housekeeping(store, nextcloud, retention_days)
...
def _handle_update(
    ...
    if session is None:
        client.send_message(chat_id, "Tiada draf aktif. Hantar /start atau /drafts.")
        return
...
def _handle_callback_query(
    ...
    if action == "generate":
        ...
        _finish_report(client, nextcloud, store, session)
...
def _finish_report(client: TelegramBotClient, nextcloud: NextcloudClient, store: DraftStore, session: Session) -> None:
    report = ReportData(
        ...
        project_location=session.data["project_location"],
    ...
    store.mark_generated(session.chat_id, session.draft_id or 0)
    ...
    client.send_message(session.chat_id, f"PDF laporan telah dijana.\nMuat turun: {share.share_url}")
...
def _cancel_session(
    ...
    if session.draft_id is not None:
        store.cancel_draft(session.chat_id, session.draft_id)
    sessions.pop(session.chat_id, None)
    client.send_message(session.chat_id, "Draf dibatalkan. Hantar /start untuk mula semula.")


def _show_drafts(client: TelegramBotClient, store: DraftStore, chat_id: int) -> None:
    drafts = store.list_drafts(chat_id)
    if not drafts:
        client.send_message(chat_id, "Tiada draf belum siap. Hantar /start untuk mula baharu.")
        return
...
def _parse_callback_data(data: str) -> tuple[str, str | int | None]:
    ...
    if prefix == REVIEW_CALLBACK_PREFIX:
        if action in {"generate", "back", "show", "add_issue", "menu_fields", "menu_edit_issues", "menu_delete_issues"}:
            return action, None
...
def _run_housekeeping(store: DraftStore, nextcloud: NextcloudClient, retention_days: int) -> None:
    cutoff = (datetime.now(timezone.utc) - timedelta(days=retention_days)).isoformat()
    expired = store.list_expired_generated_files(cutoff)
    if not expired:
        return

    LOGGER.info("Running Nextcloud housekeeping for %s generated file(s)", len(expired))
...
def _help_text() -> str:
    return (
        ...
        "- Kawasan / lokasi projek: lokasi penuh\n\n"
